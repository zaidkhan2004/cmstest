...
$('#patient_detail_table').DataTable( {
        "processing": true,
        "serverSide": true,
        "ajax": "appointment_ajax.php",
		'columnDefs': [ {
            'targets': -1,
            'data': null,
            'defaultContent': '<input type="button" id="book" value="Book Appointment">'
        }
 ...
 $('#patient_detail_table tbody').on( 'click', '#book', function () {
		var table = $('#patient_detail_table').DataTable();
        var data = table.row( $(this).parents('tr') ).data();
        window.open('appointment.php?patient_id='+data[0]);
